...
export default function PHs({ ordenes, reportes, T, s, setPhFiltro, navTo }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10 }}>
      {PHS.map(ph => {
        ...
        return (
          <button
            key={ph}
            onClick={() => { setPhFiltro(ph); navTo("ordenes"); }}
            style={{
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                {urgente > 0 && (
                  <span style={{ background: T.dangerMuted, color: T.dangerText, border: `1px solid ${T.dangerBase}33`, fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 4 }}>
                    ⚠ {urgente} urgente{urgente > 1 ? "s" : ""}
                  </span>
                )}
                {pend > 0 && (
                  <span style={{ background: T.warningMuted, color: T.warningText, border: `1px solid ${T.warningBase}33`, fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 4 }}>
                    {pend} pend.
                  </span>
                )}
                {total === 0 && !urgente && (
                  <span style={{ background: T.successMuted, color: T.successText, border: `1px solid ${T.successBase}33`, fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 4 }}>
                    OK
                  </span>
                )}
